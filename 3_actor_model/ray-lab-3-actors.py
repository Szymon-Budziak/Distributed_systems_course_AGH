...
class MethodStateCounter:
    ...
    def invoke(self, name):
        ...
        # --- now ---
        # return a random integer between 5 and 25
    ...
    def get_values_computed_by_invokers(self, names):
        # return a list of values computed by invoker name
        return [self.invokers[name] for name in names]

    def pi_sample(self, count):
        """pi_sample runs count experiments, and returns the
        ...
        for i in range(count):
            ...
            if x * x + y * y <= 1:
                in_count += 1
        return 4 * float(Fraction(in_count, count))
...
class Worker(object):
    def __init__(self, m: str, func: object):
        # type of model: lr, cl, or nn
        self._model = m
        self._func = func

    # inspect its current state and return it. For now, it could be in one of the states
    def state(self) -> str:
...
while True:
    ...
    if result:
        ...
        # If we create an actor with actor_handle = ActorClass.remote(), then when actor_handle goes out of scope and
        # is destroyed, the actor process will be terminated. Note that this only applies to the original actor handle
        # created for the actor and not to subsequent actor handles created by passing the actor handle to other tasks.
...
new_worker_invoker = MethodStateCounter.remote()

for _ in range(20):
    name = random.choice(CALLERS)
    new_worker_invoker.invoke.remote(name)

for caller in CALLERS:
    print(f"{caller} was invoked {ray.get(new_worker_invoker.get_invoker_state.remote(caller))}")

# b) - Get a list of values computed by invoker name
invoker_names = ["A", "C"]
print(f"Values computed {ray.get(new_worker_invoker.get_values_computed_by_invokers.remote(invoker_names))}")

# c) Get state of all invokers
print(f"State of all invokers: {ray.get(new_worker_invoker.get_all_invoker_state.remote())}")

# 3.2 Modify method invoke to return a random int value between [5, 25]
for _ in range(10):
    name = random.choice(CALLERS)
    print(f"Random int: {ray.get(new_worker_invoker.invoke.remote(name))}")
...
print(f"Pi value {ray.get(new_worker_invoker.pi_sample.remote(1000000))}")
...
class PiActor:
    ...
    def update_pi(self, value):
        self.pi += value
        self.iterations += 1

    def get_pi(self):
        return self.pi * 4 / self.iterations


@ray.remote
def calculate_pi(iterations, actor):
    sign = 1
    result = 0
    for i in range(iterations):
        result += sign / (2 * i + 1)
        sign *= -1
    actor.update_pi.remote(result)


actor = PiActor.remote()
tasks = [calculate_pi.remote(10000, actor) for _ in range(10000)]
ray.get(tasks)
print(f"Pi value calculated by combination of actors and tasks {ray.get(actor.get_pi.remote())}")
