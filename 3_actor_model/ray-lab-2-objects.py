...
# note that the echo function dereferences it
...
x = list(range(20))
obj_ref_x = ray.put(x)
# Echo will not automatically de-reference it
...
# Exercises 2.1) Create large lists and python dictionaries, put them in object store.
# Write a Ray task to process them.
def local_process_list(lst: list) -> None:
    print(sum(lst))


@ray.remote
def remote_process_list(lst: list) -> None:
    print(sum(lst))


def local_process_dict(dct: dict) -> None:
    print(sum(dct.values()))


@ray.remote
def remote_process_dict(dct: dict) -> None:
    print(sum(dct.values()))
...
print("Local task on large list")
cProfile.run("local_process_list(large_list)")

list_ref = ray.put(large_list)
print("Remote task on large list")
cProfile.run("ray.get(remote_process_list.remote(list_ref), timeout=6)")

print("--------------------------------------------")

print("Local task on large dict")
cProfile.run("local_process_dict(large_dict)")

dict_ref = ray.put(large_dict)
print("Remote task on large dict")
cProfile.run("ray.get(remote_process_dict.remote(dict_ref), timeout=6)")
