...
# Define a function as a Ray task to sum the contents of a np array
...
# Read both arrays.
# Use the func_name.remote(args) extension to invoke a remote Ray Task
...
# the ObjectRef owns the metadata associated and stores it in its object store.

# Ray scheduler is aware of where these object references to reside or who owns them,
...
# Add the array elements within a np.array and get the sum. Note that we are
...
@ray.remote
def remote_bubble_sort(arr: list) -> list:
    return bubble_sort(arr)


arr = [randint(1, 20) for _ in range(1000)]
